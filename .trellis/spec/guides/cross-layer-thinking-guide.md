...
## Minecraft Download URL Rewrite Contracts

Minecraft metadata and binary assets do not share the same BMCLAPI prefix.
Before constructing a mirror URL, classify the original URL by its source:

| Original source | BMCLAPI rewrite |
|---|---|
| `piston-meta.mojang.com` / `launchermeta.mojang.com` | Replace only the host; preserve the complete path (for example, `/v1/packages/<hash>/<id>.json`) |
| `piston-data.mojang.com` / `libraries.minecraft.net` | Replace the host and use the BMCLAPI `/maven` prefix where the endpoint expects Maven artifacts |
| `resources.download.minecraft.net` | Replace the host with the BMCLAPI `/assets` prefix and preserve the object path (`<prefix>/<sha1>`) |

Do not infer an asset-index endpoint from its local filename or `assetIndex.id`.
The local `assets/indexes/<id>.json` path is not the remote URL. Use the remote
`assetIndex.url` path and apply the metadata-host rewrite rule. Add a regression
test for every URL rewrite helper and verify the resulting endpoint with an
HTTP status check before changing the downloader again.
