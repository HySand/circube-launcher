...
impl Argument {
    pub fn resolve(
        ...
        launcher_features: &HashMap<String, bool>,
    ) -> Vec<String> {
        match self {
            ...
            Argument::Conditional { rules, value } => {
                if Self::evaluate_rules(rules, launcher_features) {
                    match value {
                        ...
                        ArgumentValue::Many(v) => {
                            v.iter().map(|s| Self::replace_vars(s, env)).collect()
                        }
                    }
                } else {
                    vec![]
...
pub async fn launch_minecraft(
    ...
    env.insert(
        ...
        libs_base.to_string_lossy().to_string(),
    );
