          ...
          <PopoverContent class="p-1 border border-slate-10 rounded-2xl shadow-2xl bg-white/20 backdrop-blur-md"
            ...
            <Command class="bg-transparent">
              <CommandList class="max-h-[280px] overflow-y-auto custom-scroll">
                <CommandEmpty class="py-10 text-[10px] text-center font-bold text-slate-300 tracking-widest">系统内未发现可用的
                  Java 环境</CommandEmpty>
                <CommandGroup>
        ...
        <div class="grid grid-cols-3 gap-2 px-1">
          <div class="flex flex-col gap-2">
            <Badge variant="secondary"
              ...
              系统内存</Badge>
            <span class="text-blue-800 font-numeric font-bold text-[11px] ml-0.5">{{ (totalSystemMem / 1024).toFixed(1)
              }} <small class="opacity-50">GB</small></span>
          </div>
          ...
          <div class="flex flex-col items-end gap-2">
            <Badge variant="secondary"
              ...
              剩余内存</Badge>
            <span class="text-slate-600 font-numeric font-bold text-[11px] mr-0.5">{{ (freeMemAfterAlloc /
              1024).toFixed(1) }} <small class="opacity-50">GB</small></span>
          </div>
        </div>
